...
CREATE TABLE public.lead_client_mapping (
...
CREATE INDEX idx_clients_email ON public.clients(LOWER(email)) WHERE email IS NOT NULL;
CREATE INDEX idx_clients_phone ON public.clients(phone) WHERE phone IS NOT NULL;
CREATE INDEX idx_lead_client_mapping_lead ON public.lead_client_mapping(lead_type, lead_id);
CREATE INDEX idx_lead_client_mapping_client ON public.lead_client_mapping(client_id);
...
-- Function to create client from consultation lead
...
-- Function to create client from front lead
...
-- Function to create client from full application
...
-- Function to create client from booking
...
-- Function to create client from elevate lead
...
-- Create triggers for automatic client creation
